...
逐一驱动 `compaction` 与 `contextmgr` 窗口策略的每条压缩路径。用 `t.Run` 分 6 个子用例,
...
3. `WindowPolicy_EagerCompaction_SimpleSummarizer` —— `total > CompactLimit` 触发急切压缩并注入摘要。
4. `StripLeadingOrphanToolResults` —— 截断后剥离开头的孤儿 `tool_result`。
5. `CircuitBreaker_Fallback` —— 断路器 3 次失败后开路;Manager 优雅回退为截断、不报错。
6. `RealLLMSummarizer_EagerCompaction` —— 真实 provider 生成摘要并注入(无 key 时 skip)。
